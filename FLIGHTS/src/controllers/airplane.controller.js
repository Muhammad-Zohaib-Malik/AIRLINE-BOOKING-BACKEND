...
export const createAirplane = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (error.code === 11000) {
      throw new ApiError(
        ...
        "Airplane with this name already exists"
      );
    }
    next(error);
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        airplanes,
        "Airplanes retrieved successfully"
      )
    );
});
  ...
  return res
    ...
    .json(
      new ApiResponse(
        StatusCodes.OK,
        airplane,
        "Airplane retrieved successfully"
      )
    );
});
...
export const deleteAirplaneById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(StatusCodes.OK)
    .json(
      new ApiResponse(StatusCodes.OK, airplane, "Airplane deleted successfully")
    );
});
...
export const updateAirplaneById = asyncHandler(async (req, res) => {
  ...
  const airplane = await Airplane.findByIdAndUpdate(id, validatedData, {
    new: true,
  });
  if (!airplane) {
    throw new ApiError(StatusCodes.NOT_FOUND, "Airplane not found");
  ...
  return res
    .status(StatusCodes.OK)
    .json(
      new ApiResponse(StatusCodes.OK, airplane, "Airplane updated successfully")
    );
});
